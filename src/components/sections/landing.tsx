import LandingSectionAd from '../landing-section/landingSectionAd'
import Item from '../landing-section/item'

async function getData() {
  const res = await fetch('https://competition-e-commerce-backend-1.vercel.app/categories/dresses-and-suits')
  ...
  return (
    <section className='text-gray-800 font-semibold py-16 w-full max-w-[1300px] flex flex-col gap-8'>
      ...
      <section className='grid grid-cols-2 lg:grid-cols-4 gap-6'>
        {
          items?.slice(0, 4).map(e => <Item itemData={e} />)
        }
      </section>
      <section className='lg:flex gap-6'>
        <LandingSectionAd/>
        <div className='lg:w-1/2 grid grid-cols-2 gap-6'>
          {items?.slice(0, 2).map(e => <Item itemData={e} />)}
        </div>
      </section>
    </section>
  )
